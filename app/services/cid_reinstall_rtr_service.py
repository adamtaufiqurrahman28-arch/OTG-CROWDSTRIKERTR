...
class CIDReinstallRTRService:
    ...
    2. mkdir C:\\ProgramData\\CSMigration
    3. cd C:\\ProgramData\\CSMigration
    4. put WindowsSensor.exe
    5. put CsUninstallTool.exe
    6. put migrate_falcon_sensor.bat
    7. run migrate_falcon_sensor.bat -CommandLine="<DESTINATION_CID>"
    """
    ...
    @classmethod
    def has_host_errors(cls, resp: Dict[str, Any]) -> bool:
        per_host = cls.extract_per_host_results(resp)

        for _, data in per_host.items():
    ...
    @classmethod
    def get_host_error_summary(cls, resp: Dict[str, Any]) -> str:
        per_host = cls.extract_per_host_results(resp)
        messages = []
        ...
        for aid, data in per_host.items():
            ...
            for err in errors:
                code = err.get("code", "")
                msg = err.get("message", "")
                messages.append(f"{aid}: {code} - {msg}")
    ...
    def run_basic_command(
        ...
        """
        Untuk RTR basic command:
        mkdir, cd, ls, pwd, dll.

        Note:
        Falcon RTR API biasanya butuh:
        - base_command='mkdir'
        - command_string='mkdir C:\\ProgramData\\CSMigration'

        Jadi command_string tetap membawa nama command-nya.
        """

        resp = self.rtr.batch_active_responder_command(
        ...
        if resp.get("status_code") not in (200, 201):
            raise RuntimeError(
                f"batch_active_responder_command gagal. "
                f"base_command={base_command}, command_string={command_string}, body={resp.get('body')}"
            )

        if self.has_host_errors(resp):
            raise RuntimeError(
                f"RTR basic command failed. "
                f"base_command={base_command}, command_string={command_string}, "
                f"errors={self.get_host_error_summary(resp)}"
            )
    ...
    def run_admin_command(
        ...
        """
        Untuk RTR admin command:
        put, run, put-and-run.

        Note:
        Sama seperti project lama, command_string dibuat full command.
        Contoh:
        - base_command='put'
        - command_string='put WindowsSensor.exe'

        - base_command='run'
        - command_string='run migrate_falcon_sensor.bat -CommandLine="CID"'
        """
    ...
    def mkdir(
        ...
        return self.run_basic_command(
            batch_id=batch_id,
            host_ids=host_ids,
            base_command="mkdir",
            command_string=f"mkdir {workdir}",
        )
    ...
    def run_bat(
        ...
        bat_path = rf"C:\ProgramData\CSMigration\{bat_name}"

        command = f'run cmd.exe -CommandLine="/c {bat_path} {destination_cid}"'
        ...
        steps = [
            ...
            (
                ...
                lambda: self.put(
                    batch_id=batch_id,
                    host_ids=host_ids,
                    put_file_name=uninstall_tool_name,
                    queue_offline=queue_offline,
                ),
            ),
            (
